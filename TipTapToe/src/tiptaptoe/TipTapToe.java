...
class GameSystem {
    ...
    private void showPlayer2Dialog(JFrame parent) {
    ...
    private void registerPlayer(String username, String password, String name, int points) {
...
class Game {
    ...
    public void showGameBoard() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                final int row = i;
                final int col = j;
                buttons[i][j].addActionListener(e -> makeMove(row, col, buttons));
    ...
    private void makeMove(int row, int col, JButton[][] buttons) {
        if (board[row][col] != '-') {
            JOptionPane.showMessageDialog(gameFrame, "Casilla ocupada. Elige otra.", "Movimiento inválido", JOptionPane.WARNING_MESSAGE);
            return;
        }
